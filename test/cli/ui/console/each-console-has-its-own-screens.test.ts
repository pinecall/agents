...
describe("the gateway's console", () => {
  it("looks at production, and runs the org", () => {
    ...
    expect(names(ORG_SCREENS, "hosted")).toEqual(["Home", "Overview", "Live", "Sessions", "Evals", "Memory", "Usage", "Numbers", "Tokens", "Providers", "Team", "Lexicon"]);
  });
...
describe("a machine's own console", () => {
  it("looks at the sandbox, and has none of the org's accounts", () => {
    ...
    expect(names(ORG_SCREENS, "local")).toEqual(["Home", "Overview", "Live", "Sessions", "Evals", "Memory", "Phone testing", "Lexicon"]);
  });

  it("has every screen of an agent, Dev chat among them", () => {
    expect(names(AGENT_SCREENS, "local")).toEqual(["Talk", "Chat", "Dev chat", "Calls", "Sessions", "Settings", "Pipeline", "Knowledge", "Memory", "Evals", "Widget"]);
  });
