...
/** What must come of one call: one field is one of the four ways a hang-up costs a business. */
export interface Expected {
  /** Every category named got at least one fact. The class's own `memory.remember` words. */
  writes?: string[];
  /** No fact was written under any of these — the class's own `memory.forget` words. */
  never?: string[];
  /** The sharper half: no fact CARRIES one of these values, under whatever category. */
  never_says?: string[];
  /** Every held fact named here was superseded, and no other held fact was. */
  invalidates?: string[];
}

/** One call written down and what memory must make of it. Both speakers: nothing is re-run. */
export interface Case {
  name: string;
  said: [string, string][];
  holds?: string[];
  /** Sentences somebody tried to get into memory. Planting one IS the assertion. */
  plants?: string[];
  channel?: "phone" | "web" | "whatsapp";
  expect?: Expected;
}

/** One case as the gateway judged it: what memory would have kept, and what did not hold. */
export interface Judged {
  name: string;
  held: boolean;
  wrote: string[];
  refused: string[];
  broke: { check: string; detail: string }[];
}

/** A whole run: which model answered, how many cases held, and every one of them. */
...
export async function run(argv: string[], how: Running = {}): Promise<number> {
  ...
  const cases = matching(await casesIn<Case>(positionals, CASES), values.grep);
  if (cases.length === 0) {
...
export async function extracted(door: Door, agent: string, cases: Case[]): Promise<Answer> {
  return await asked<Answer>(door, `/v1/agents/${encodeURIComponent(agent)}/memory/extraction`, {
    method: "POST",
    body: { cases },
  });
}
...
export function linesOf(answer: Answer): string[] {
  ...
  for (const result of answer.results) {
    ...
    const width = Math.max(...result.broke.map((one) => one.check.length));
    lines.push(`  ${BROKEN} ${result.name}`);
    for (const one of result.broke) lines.push(`      ${one.check.padEnd(width)}  ${one.detail}`);
