/** ◎ what memory recalled and ¶ what retrieval found for a turn: the two rows a fill leaves between the turns. */
...
export function MemoryRow({ ops, seq }: { ops: MemoryOps; seq: number }): ReactNode {
  return <FillRow glyph="◎" said={memoryLine(ops)} lines={factLines(ops)} seq={seq} />;
}
...
export function SourcesRow({ sources, seq }: { sources: DocsSources; seq: number }): ReactNode {
  return <FillRow glyph="¶" said={sourcesLine(sources)} lines={sourceLines(sources)} seq={seq} />;
}
...
function FillRow({ glyph, said, lines, seq }: { glyph: string; said: string; lines: string[]; seq: number }): ReactNode {
  return (
