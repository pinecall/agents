...
| `voices.ts` | a vendor's voices in a language, the id first; and one said by the gateway — the protocol's `VoiceSample`, no words means the gateway's own line — played on this machine from a file, kept where `--save` says, with its first-audio time when the gateway timed it |
