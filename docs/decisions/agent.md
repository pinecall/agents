...
dependency of this package, which pnpm then links a `tsc` bin for, shadowing the 7 the repo pins
and forcing `build` and `lint` to name a `tsc` by path. Two TypeScripts, and a shadowed bin, for
one parse.
...
`tsconfig.tenant.json`, the preset a tenant extends. The example's tsconfig now
reads `"extends": "pinecall/tsconfig.tenant.json"` and declares no compiler flag of
...
- `tsconfig.tenant.json` — `experimentalDecorators` comes out, and with it the
