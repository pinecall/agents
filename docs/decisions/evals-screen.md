...
The screen belongs to the console. It was written as a tombstone on 2026-09-08,
...
itself on 127.0.0.1. `src/cli/ui/console/screens/evals/` and `screens/sessions/` are where they were.
...
See the runtime's `docs/decisions/scoring.md`.
...
`judges`: every judge that was RUN, answered or not, written by `judging/score.py` on every path,
`[]` when nothing judged the call at all. `judging/score.py:_the_judges_of` is the one definition
